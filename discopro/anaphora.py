from lambeq.backend.grammar import Word, Id, Ty, Cup, Cap, Diagram
from discopro.grammar import words_and_cups, is_pregroup

def _interchange(diag: Diagram, i: int, j: int, left=False):
    if any(len(list(layer)) != 3 for layer in diag.layers):
        raise NotImplementedError
    if not 0 <= i < len(diag) or not 0 <= j < len(diag):
        raise IndexError
    if i == j:
        return diag 
    if j < i - 1:
        result = diag
        for k in range(i - j):
            result = _interchange(result, i-k, i-k-1, left=left)
        return result 
    if j > i + 1:
        result = diag 
        for k in range(j - i):
    ...
    if j < i:
        i, j = j, i
    off0, off1 = diag.offsets[i], diag.offsets[j] 
    left0, box0, right0 = diag.layers[i] 
    left1, box1, right1 = diag.layers[j] 
    ...
    if left and off1 >= off0 + len(box0.cod): # box0 left of box1
        ...
        raise ValueError
    return diag[:i] >> layer1 >> layer0 >> diag[i+2:]
...
def _insert_cup_min_swaps(diag, left, right, cup):
    ...
    for target_idx, local_box_idx in enumerate(local_boxes_idx):
        ...
        diag = _interchange(diag, local_box_idx, target_idx)
    ...
    local_boxes = Diagram(local_boxes.dom[left:right+1],
                          local_boxes.cod[left:-(n_diag_dom-right)+1],
                          [*local_boxes])
    for layer in local_boxes.layers:
        layer.left = layer.left[:-left]
        layer.right = layer.right[:-left]
...
def connect_anaphora_on_top(diag, pro_box_idx, ref_box_idx, use_cap=False):
    ...
    left_words = Id(Ty()).tensor(*left_words_boxes) if left_words_boxes else Id(Ty())
    mid_words = Id(Ty()).tensor(*mid_words_boxes) if mid_words_boxes else Id(Ty())
    right_words = Id(Ty()).tensor(*right_words_boxes) if right_words_boxes else Id(Ty())
