from lambeq.backend.grammar import Diagram, Id, Ty, Cup, Cap, Swap, Word, Functor
from lambeq.rewrite import SimpleRewriteRule
from lambeq.core.types import AtomicType
#from discopy.monoidal import Swap as monoidal_Swap
from lambeq.backend.grammar import Swap as monoidal_Swap
...
def _try_contract_leaf(diag):
    ...
    for self_box_idx, self in enumerate(diag.boxes):
        ...
        return Diagram(diag.dom, diag.cod, new_boxes), True
    return diag, False
...
def _try_contract_bending(diag):
    for self_box_idx, self in enumerate(diag.boxes):
        ...
        new_diag = Diagram(diag.dom, diag.cod, new_boxes)
